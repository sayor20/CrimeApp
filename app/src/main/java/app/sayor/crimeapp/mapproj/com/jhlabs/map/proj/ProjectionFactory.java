...
import app.sayor.crimeapp.mapproj.com.jhlabs.map.*;
import app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.*;
...
public class ProjectionFactory {
	 ...
	 */
	public static app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection fromPROJ4Specification(Context baseContext, String[] args ) {
		app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection projection = null;
		app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Ellipsoid ellipsoid = null;
		double a = 0, b = 0, es = 0;
		...
		if ( s != null ) {
			projection = getNamedPROJ4CoordinateSystem(baseContext, s );
			if ( projection == null )
				throw new ProjectionException( "Unknown projection: "+s );
		...
		if ( s != null ) 
			...
			if ( s != null ) {
				app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Ellipsoid[] ellipsoids = app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Ellipsoid.ellipsoids;
		...
		projection.setEllipsoid( new app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Ellipsoid( ellipsoidName, a, es, ellipsoidName ) );
...
//		projection.setProjectionLatitude2Degrees( 0 );
		...
		if ( s != null ) 
			projection.setProjectionLatitudeDegrees( (float) parseAngle( s ) );
		s = (String)params.get( "lon_0" );
		if ( s != null ) 
			projection.setProjectionLongitudeDegrees( (float) parseAngle( s ) );
		s = (String)params.get( "lat_1" );
		if ( s != null ) 
			projection.setProjectionLatitude1Degrees( (float) parseAngle( s ) );
		s = (String)params.get( "lat_2" );
		if ( s != null ) 
			projection.setProjectionLatitude2Degrees( (float) parseAngle( s ) );
		s = (String)params.get( "lat_ts" );
		if ( s != null ) 
			projection.setTrueScaleLatitudeDegrees( (float) parseAngle( s ) );
		s = (String)params.get( "x_0" );
		if ( s != null ) 
			projection.setFalseEasting( (float) Double.parseDouble( s ) );
		s = (String)params.get( "y_0" );
		if ( s != null ) 
			projection.setFalseNorthing( (float) Double.parseDouble( s ) );

		s = (String)params.get( "k_0" );
		if ( s == null ) 
			s = (String)params.get( "k" );
		if ( s != null ) 
			projection.setScaleFactor( (float) Double.parseDouble( s ) );

		s = (String)params.get( "units" );
		if ( s != null ) {
			Unit unit = Units.findUnits( s );
			if ( unit != null )
				projection.setFromMetres( (float) (1.0 / unit.value ));
		}
		s = (String)params.get( "to_meter" );
		if ( s != null ) 
			projection.setFromMetres( (float)( 1.0/Double.parseDouble( s )));
...
//pm
	...
	static app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection getNamedPROJ4Projection(String name ) {
		if ( registry == null )
			initialize();
		Class cls = (Class)registry.get( name );
		if ( cls != null ) {
			try {
				app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection projection = (app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection)cls.newInstance();
				if ( projection != null )
	...
	static void initialize() {
	    ...
	    register( "alsk", app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection.class, "Mod. Stereographics of Alaska" );
		register( "apian", app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection.class, "Apian Globular I" );
		register( "bacon", app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection.class, "Bacon Globular" );
		register( "cea", app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection.class, "Equal Area Cylindrical" );
...
//		register( "wink2", Projection.class, "Winkel II" );
	...
	public static app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection readProjectionFile(Context baseContext, String file, String name ) throws IOException {
		//BufferedReader reader = new BufferedReader( new InputStreamReader( ProjectionFactory.class.getResourceAsStream( "/nad/"+file ) ) );

        AssetManager assManager = baseContext.getApplicationContext().getAssets();
        InputStream is = null;
        try {
            is = assManager.open("nad83");
        } catch (IOException e) {
        ...
        //BufferedInputStream reader = new BufferedInputStream( new FileInputStream("/assets/nad83"));
		...
		while ( t.ttype == '<' ) {
			...
			if ( cname.equals( name ) ) {
				...
				v.copyInto( args );
				reader.close();
				return fromPROJ4Specification( baseContext, args );
			}
		}
		reader.close();
		return null;
	...
	public static app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection getNamedPROJ4CoordinateSystem(Context BaseContext, String name ) {
		String[] files = {
			"world",
			"nad83",
			"nad27",
		...
		try {
            int p = name.indexOf(':');
            if ( p >= 0 )
                return readProjectionFile( BaseContext, name.substring(0,p), name.substring(p+1) );
			for ( int i = 0; i < files.length; i++ ) {
				app.sayor.crimeapp.mapproj.com.jhlabs.map.proj.Projection projection = readProjectionFile( BaseContext, files[i], name );
				if ( projection != null )
					return projection;
			}
		}
...
/*		Projection projection = ProjectionFactory.fromPROJ4Specification(get args );
		if ( projection != null ) {
				...
				if ( !arg.startsWith("+") && !arg.startsWith("-") ) {
					try {
						...
						while ( (line = reader.readLine()) != null ) {
							...
							p.x = (float) format.parse( slon, null ).doubleValue();
							p.y = (float) format.parse( slat, null ).doubleValue();
