...
package app.sayor.crimeapp.mapproj.com.jhlabs.map;
...
public class MapMath {
	...
	public static float normalizeLatitude(float angle) {
...
*/
	...
	public static int intersectSegments(PointF aStart, PointF aEnd, PointF bStart, PointF bEnd, PointF p) {
		...
		num = b1*c2 - b2*c1;
		p.x = (float)( (num < 0 ? num-offset : num+offset) / denom);

		num = a2*c1 - a1*c2;
		p.y = (float)( (num < 0 ? num-offset : num+offset) / denom);
	...
	public static PointF perpendicular(PointF a) {
	...
	public static void negate(PointF a) {
		a.x = -a.x;
		a.y = -a.y;
	}
